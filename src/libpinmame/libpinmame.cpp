...
static_assert(PINMAME_MEMMAP_TYPE_INT64 == CTLPI_STATE_FORMAT_INT64,
	"PINMAME_MEMMAP_TYPE_INT64 must track CTLPI_STATE_FORMAT_INT64");
static_assert(PINMAME_MEMMAP_TYPE_STRING == CTLPI_STATE_FORMAT_STRING,
	"PINMAME_MEMMAP_TYPE_STRING must track CTLPI_STATE_FORMAT_STRING");
...
PINMAMEAPI int PinmameGetMemMapStateInfo(const unsigned int index, PinmameMemMapStateInfo* const p_info)
{
	...
	p_info->typeMask = (int)state.type;
	return 0;
}
...
PINMAMEAPI int PinmameGetMemMapStateInt(const unsigned int index, int64_t* const p_value)
{
	...
	const auto& state = msgLocals.memMapStates[index];
	if (state.type != PINMAME_MEMMAP_TYPE_INT64)
		return -1;

	return state.getState(index, p_value) ? 0 : -1;
}
...
PINMAMEAPI int PinmameGetMemMapStateString(const unsigned int index, char* const p_buffer, const int bufferSize)
{
	...
	const auto& state = msgLocals.memMapStates[index];
	if (state.type != PINMAME_MEMMAP_TYPE_STRING)
		return -1;
	...
	if (!state.getState(index, &p_str) || p_str == nullptr)
		return -1;
