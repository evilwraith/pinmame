   ...
   traverse_and_collect = [&traverse_and_collect, &parseAddress, &jsonFlagIsSet, &jsonFlagIsClear, &memRegions, &charMap, &sharedValueLists, isLittleEndianPlatform](const json& node, const std::string& group, const std::map<std::string, json>& inherited_fields) {
      ...
      if (node.contains("offsets") || node.contains("start"))
      {
         ...
         if (encoding == "int" || encoding == "bcd" || encoding == "bits" || encoding == "bool" || encoding == "enum")
         {
            ...
            typeMask = enumIsString ? CTLPI_STATE_TYPE_STRING : CTLPI_STATE_TYPE_INT64;
            getter = [offsets, nibble, byteMask, isBCD, isBool, isReversed, isLittleEndian, valueScale, valueOffset, enumValues, enumIsString](unsigned int index, int type, void* pResult)
               {
                  int64_t v = 0;
                  if (type != (enumIsString ? CTLPI_STATE_TYPE_STRING : CTLPI_STATE_TYPE_INT64))
                     return -1;
                  ...
                  // not cover has no meaning, so it is reported as unreadable rather than as its
                  // own number -- that is the case the maps use to mark a state invalid.
                  if (!enumValues.empty())
                  {
...
{
	// Map files come from disk, and on this fork the directory is user-overridable,
	// so a truncated or hand-edited file is reachable input rather than a build bug.
	// nlohmann::json signals that by throwing, which would cross this C boundary and
	// terminate the host, so failure is contained here and reported as "no states".
	try {
